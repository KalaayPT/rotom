...
pub fn run_init_with_options(
    ...
    if !config_path.exists() {
        ...
        let content = toml::to_string_pretty(&config).map_err(|source| ProjectError::SerializeConfig { source })?;
        fs::write(
            &config_path,
            format!(
...
fn maybe_convert_existing_sources(
    ...
    if answer.is_empty() || answer == "y" || answer == "yes" {
        Ok((convertible_files_detected, convert_project(root, config, false)?.converted))
    } else {
